...
```{r}
write.table(geneCor, paste0("./annotation/correlation_tables/", ME, "_geneCorrelation.txt"), sep="\t")
```
...
for i, comm in enumerate(louvain_comms):
    if len(comm)>2: #ensure no single nodes
        subgraph=nw.subgraph(comm)
        
        nx.draw(subgraph, with_labels=False, node_color=f"C{i}", node_size=100, font_size=6, edge_color="gray", linewidths=1, edgecolors='black')
        plt.show()
...
Community clustering presents finer grain clusters,whcih are more biologically relevant than the ones calculated by KMeans and DBscan
