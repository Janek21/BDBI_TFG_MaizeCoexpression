...
plot(pca_data$`Cumulative Proportion`, xlab="PC component number", ylab="Cumulative variance", main="Elbow plot for PC component cutoff")
abline(v=pc_cutoff, col="red", lwd=2)
```
...
plot(1:20, total_means, xlab="Distance between clusters", type="b")
abline(v=1, col="red")
abline(h=total_means[length(total_means)], col="red")
```
...
plot(shortened_pca$x, col=kres$cluster, lwd=3)
```
...
Community(louvain) clustering delivered better, finer grain clusters, more biologically relevant than KMeans and DBscan

# Saving the network

```{r}
#write.table(geneCor, paste0("./annotation/ME_correlation/", ME, "_geneCorrelation.txt"), sep="\t")
## write table of umap cluster, how? what do i get frm it?
```
