...
Run expression analysis for the data
...
```{r rawUMAP}
raw_umap_data<-umap(repl_dvData)

ggplotly(dataPlotter(raw_umap_data, repl_meta)+ggtitle("Raw data Umap"))
```
...
ggplotly(dataPlotter(sc_Numap_data, repl_meta)+ggtitle("Scaled data Umap"))
```

```{r scUMAP_alt}
umap_data<-sc_Numap_data

ggplotly(ggplot(as.data.frame(umap_data$layout), aes(x=umap_data$layout[,1], y=umap_data$layout[,2], color=repl_meta$org_location))+
  geom_point(size=5)+
  scale_shape_manual(values = c(19, 1, 2, 15, 8))+
...
```
```{r}
FinalUmap<-umap(Nrepl_dvData_sc, n_neighbors=29, n_epochs=450, min_dist=0.1) #nneighbor:22, 29, 35 ##rework depending on normalization
#Modify min_dist as 0.1 fits better the data in this case

ggplotly(dataPlotter(FinalUmap, repl_meta)+
