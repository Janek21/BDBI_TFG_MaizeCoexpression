...
hist(Nrepl_data[,seq(1,149, by=15)], breaks=100)
```
...
```{r RPKMlExpRm}
#Filter low expression genes
keep<-rowSums(Nrepl_data>=0) #as most lowly expressed genes are slightly above 0(=1), so we filter starting at 1 rep and more than 5 total in samples
Nrepl_data<-Nrepl_data[keep,]
...
umap_data<-umap(Nrepl_UMAP)

ggplot(as.data.frame(umap_data$layout), aes(x=umap_data$layout[,1], y=umap_data$layout[,2], shape=repl_meta$species ,color=repl_meta$org_location))+
  geom_point(size=6)+
...
umap_data<-umap(scale(Nrepl_UMAP))

ggplot(as.data.frame(umap_data$layout), aes(x=umap_data$layout[,1], y=umap_data$layout[,2], color=repl_meta$org_location))+
  geom_point(size=6)+
  scale_shape_manual(values = c(19, 1, 2, 15, 8))+
...
ggplot(as.data.frame(umap_data$layout), aes(x=umap_data$layout[,1], y=umap_data$layout[,2], shape=repl_meta$species, color=repl_meta$org_location))+
  geom_point(size=6)+
  scale_shape_manual(values = c(19, 1, 2, 15, 8))+
  theme_minimal()+ggtitle("Species+tissue based")
