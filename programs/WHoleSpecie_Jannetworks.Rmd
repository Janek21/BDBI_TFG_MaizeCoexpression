...
#dev.off()
```
# Expression data analysis {.tabset}
...
The modules will present a good visualization of the effect by specie.
...
```{r}
geneModule_df<-data.frame(modules=ModNetwork$colors)
#write.table(geneModule_df, file="geneModule.txt")

ME_gene_ensembl <- names(ModNetwork$colors)[ModNetwork$colors == "darkturquoise"]
MEgene_expression <- as.data.frame(Nrepl_data[, colnames(Nrepl_data) %in% ME_gene_ensembl])
```
...
library(org.Zmays.eg.db)  # Maize annotation package
library(DOSE)             # Optional, for visualization
```
...
zmays_dataset <- grep("zmays", datasets$dataset, value = TRUE, ignore.case = TRUE)

#mart <- useDataset("zmays_eg_gene", useMart("ensembl"))
...
genes <- getBM(
 ...
 ####
ensembl_maize <- useDataset(dataset = zmays_dataset, mart = ensembl_plants)
entrez_mapping <- getBM(
  attributes = c("ensembl_gene_id", "entrezgene_id"),
  filters = "ensembl_gene_id",
  values = ME_gene_ensembl,
  mart = ensembl_maize
)

listAttributes(ensembl_maize)[,1:2]
