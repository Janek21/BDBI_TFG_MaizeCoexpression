...
#Filter low expression genes
#keep<-rowSums(Nrepl_data>1)>=12 #as most lowly expressed genes are slightly above 0(=1), so we filter starting at 1 rep and more than 10 total in samples
keep<-(rowSums(Nrepl_data>1)>=14)|(apply(Nrepl_data, 1, max)>10) #total larger than 10 or 1 value larger than 5
Nrepl_data<-Nrepl_data[keep,]
...
#Nrepl to data frame
Nrepl_data<-as.data.frame(Nrepl_data) #evetually transpose
hist(Nrepl_data[,c(seq(1,149, by=15))])
```
...
keep<-filterByExpr(Nrepl_data, min.total.count=10, min.count=5)
Nrepl_data<-Nrepl_data[keep,]
...
```{r RPKMlExpRm}
#Filter low expression genes
keep<-(rowSums(Nrepl_data>0)>=13)|(apply(Nrepl_data, 1, max)>11) #total larger than 10 or 1 value larger than 5
