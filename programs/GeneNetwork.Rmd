...
```{r dtLoad}
dataNL<-read.delim("../data/all5gt.nolen.csv", row.names=1, stringsAsFactors=TRUE)
metadata<-read.delim("../data/sample.tissue.correct.cluadj.txt", header=FALSE, row.names=1, stringsAsFactors=TRUE)
...
#@@ maybe random sample for 15 (lowest value) and reduce all to lowest?
...
CorLevelPlot(heatmap.data,
             x = names(heatmap.data)[columns_number/2:columns_number],
             y = names(heatmap.data)[1:(columns_number/2)-1],
             rotLabX = 45,
             cexCorval = 0.4, cexLabY = 0.5, cexLabX = 0.7
            )

#@@ export with genes? or with samples -> genes is better
exportNetworkToCytoscape(bwnet,
...
module.membership.measure <- cor(module_eigengenes, t(N_dataNL), use = 'p')
...
gene.signf.corr <- cor(t(N_dataNL), bin_metadata$sp_B73, use = 'p')
