...
#EP1_23_4 is a clear outlier, we dont care about outilers
```
...
cor<-temp_cor
saveRDS(bwnet, file = "./bwnet_B73.RDS")
```


```{r nwLoad}
#saveRDS(bwnet, file = "./bwnet_signed.RDS")
bwnet<-readRDS("./bwnet_B73.RDS")
```
...
bin_metadata<-cbind(rep_bin, loc_bin)
#@@? VV
