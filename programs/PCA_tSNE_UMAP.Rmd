...
##
Ndata<-as.data.frame(t(Nrepl_data))
...
plotly::ggplotly(dataPlotter(N_umap, metadata)+ggtitle("Normalized Data UMAP (No Outlier)"))
```
...
neighbors<-seq(15, 40, by=7) #15
#neighbors<-c(20, 23, 27, 30)

for (i in seq_along(neighbors)){
  N<-neighbors[i]
  Nplot<-neighborUMAP(Ndata, repl_meta, N)
  NPlot_list[[i]]<-Nplot
}
...
plotly::ggplotly(neighborUMAP(Ndata, metadata, 17))
plotly::ggplotly(neighborUMAP(Ndata, metadata, 24))
#@@
```
...
plotly::ggplotly(epochUMAP(Ndata, metadata, 500)) 
#@@70
```
...
FinalUmap<-umap(Ndata, n_epochs=500, min_dist=0.1, n_neighbors=24)
