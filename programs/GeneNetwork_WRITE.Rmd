...
gc()
saveRDS(TOM, file="./TOMmatrix.RDS")
```


```{r}
TOM<-readRDS("./TOMmatrix.RDS")
gc()
...
saveRDS(genes, file="./colnameGENES.RDS")
gc()
```
...
genes<-readRDS("./colnameGENES.RDS")
adjacency<-readRDS("./TOMmatrix.RDS")
gc()
...
saveRDS(edge_list, file="./edge_list.RDS")
```
...
saveRDS(edge_list, file="./edge_list2.RDS")
```


```{r}
edge_list<-readRDS("./edge_list2.RDS")
gc()
edge_list <- edge_list[!duplicated(t(apply(edge_list, 1, sort))), ]
saveRDS(edge_list, file="./edge_list3.RDS")
```


```{r}
edge_list<-readRDS("./edge_list3.RDS")
gc()
