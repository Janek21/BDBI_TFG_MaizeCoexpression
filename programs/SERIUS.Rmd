...
# Clustering
```{r}
kmeans_res<-kmeans(Nrepl_data, 3)
...
#distance betweeen clusters is reduced as nº of clusetrs increases

kmeans_res<-kmeans(geneCor, 5)
...
library(NbClust)
set.seed(42)
scaled_data <- geneCor  # Samples in rows, features=genes
#scaled_data<-Nrepl_data
...
plot(scale(Nrepl_data), col=kmeans_clusters)

plot(Ndt_umap$layout, col=kmeans_clusters)
```
...
library(ggraph)
library(igraph)
adjacency_matrix <- abs(geneCor) > 0.3
