...
module_percent <- round(100 * module_counts / sum(module_counts), 1)

# Create labels with percentages
labels_with_pct <- paste0(names(module_counts), " (", module_percent, "%)")

# Create a named vector for labels
names(labels_with_pct) <- names(module_counts)

# Convert module to factor with levels in the order of counts
color$module <- factor(color$module, levels = names(module_counts))

# Create a named vector for colors (assuming ModNetwork$colors are actual colors)
# Here, assuming the colors are stored in the levels of module or similar
# For example, if your modules are colors themselves:
module_colors <- names(module_counts)  # or assign actual colors if different

ggplot(color, aes(x=module, fill=module)) +
  geom_bar() +
  scale_fill_manual(values = module_colors, labels = labels_with_pct) +
...
ggplot(long_data, aes(x=Trait, y=Module, fill=Correlation)) +
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1, size=10, color="black"),
        ...
        panel.grid.minor=element_line(color="black", linewidth=3))+
  coord_fixed()
```
