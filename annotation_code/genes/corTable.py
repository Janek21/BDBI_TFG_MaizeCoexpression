...
def main():
    ...
    for i, comm in enumerate(louvain_comms):
        if len(comm)>2:
            #if gint in comm:
                print(comm)
                dt=gan.main(module, comm, 10, 10)
                subgraph=nw.subgraph(comm)
                s_pos=nx.spring_layout(subgraph, seed=42)
                
                plt.title(f"{len(comm)} genes\n Known: {len(comm)-len(dt.keys())} Unknown:{len(dt.keys())}")
                ...
                print(dt.values())
                unknown_genes={node: node for node in subgraph.nodes if node in dt.keys()}
                k_genes={node: node for node in subgraph.nodes if node in dt.values()}
                #nx.draw_networkx_labels(subgraph, pos=s_pos, labels=unknown_genes, font_color="red")
                nx.draw_networkx_nodes(subgraph, pos=s_pos, nodelist=unknown_genes, node_color="black", node_size=100)
                nx.draw_networkx_nodes(subgraph, pos=s_pos, nodelist=k_genes, node_color=f"red", node_size=100)
